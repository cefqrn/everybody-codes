...
from itertools import chain, compress, cycle, islice, pairwise, combinations, product
...
def solve(data):
    ...
    best_test_case = []
    for x in free_branches:
        if x in mined:
            best_test_case.append(0)
        else:
            best_test_case.append(1)
    ...
    conflicts = tuple(maxed & mined)
    if conflicts:
        best = -inf

    for assignment in product((0, 1), repeat=len(conflicts)):
        for k in compress(conflicts, assignment):
            best_test_case[free_branches.index(k)] = 1

        best = max(best, with_case(tuple(best_test_case)))

        for k in compress(conflicts, assignment):
            best_test_case[free_branches.index(k)] = 0

    result = 0
    for test_case in test_cases:
        if with_case(test_case):
            result += best - with_case(test_case)
    return result
