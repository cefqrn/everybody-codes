...
def solve(data):
    ...
    grid1, lines1, start, end1 = data
    grid2, lines2, _,     end2 = parse("\n".join(rotated(lines1)))
    grid3, lines3, _,     end3 = parse("\n".join(rotated(lines2)))
    ...
    connections = [defaultdict(list) for _ in range(3)]
    for i, (g1, g2) in enumerate(pairwise([grid1, grid2, grid3, grid1])):
        for (x, y), c in g1.items():
            if c != "T":
                continue

            if g2.get((x, y)) == "T":
                connections[i][x, y].append((x, y))

            if g2.get((x+1, y)) == "T":
                connections[i][x, y].append((x+1, y))
            if g2.get((x-1, y)) == "T":
                connections[i][x, y].append((x-1, y))
                # connections[i][x+1, y].append((x, y))

            if (x ^ y) & 1:
                if g2.get((x, y+1)) == "T":
                    connections[i][x, y].append((x, y+1))
            else:
                if g2.get((x, y-1)) == "T":
                    connections[i][x, y].append((x, y-1))
                    # connections[i][x, y+1].append((x, y))
    ...
    while left:
        ...
        print(p, t, connections[t%3][p])

        for o in connections[t%3][p]:
            heappush(left, (t+1, o))
