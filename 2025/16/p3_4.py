...
@contextmanager
def blocks_placed_on(wall, number):
    for column in columns_affected_by(number, len(wall)):
        wall[column] -= 1
...
def spell_required_for(wall, first_number=1):
    ...
    # if any of the columns would go negative
    if not all(
            map(partial(getitem, wall),
            columns_affected_by(first_number, len(wall)))):
        return None

    with blocks_placed_on(wall, first_number):
...
def solve(wall):
    spell = spell_required_for(list(wall))
