...
def solve_p2(data):
    ...
    isolated_b = isolated - isolated_a

    return len(isolated_a), len(isolated_b)
...
def solve(data):
    ...
    tile_w = len(vertical_offsets  ) * 2
    tile_h = len(horizontal_offsets) * 2

    h_tiles = actual_width // tile_w
    v_tiles = actual_height // tile_h

    print(tile_w, tile_h, actual_width, actual_height)
    print(h_tiles, v_tiles)

    a = b = 0

    if h_tiles and v_tiles:
        c, d = solve_p2((tile_w, tile_h, horizontal_offsets, vertical_offsets))
        a += c * h_tiles * v_tiles
        b += d * h_tiles * v_tiles

    if h_tiles:
        c, d = solve_p2((tile_w, actual_height % tile_h, horizontal_offsets, vertical_offsets))
        a += c * h_tiles
        b += d * h_tiles

    if v_tiles:
        c, d = solve_p2((actual_width % tile_w, tile_h, horizontal_offsets, vertical_offsets))
        a += c * v_tiles
        b += d * v_tiles

    c, d = solve_p2((actual_width % tile_w, actual_height % tile_h, horizontal_offsets, vertical_offsets))
    a += c
    b += d
